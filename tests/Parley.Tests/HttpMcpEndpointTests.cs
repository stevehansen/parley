...
public class HttpMcpEndpointTests
{
    ...
    {
        var r = await _mcp.HandleAsync(CallTool("send_message", new { topic = "work", content = "hi" }), "backend", null, null, default);
    ...
    {
        var init = await _mcp.HandleAsync(Rpc("initialize", new { protocolVersion = "2025-06-18", roots = new[] { new { uri = "file:///P:/Frontend" } } }), null, null, null, default);

        init.McpSessionId.ShouldNotBeNull();
        Parse(init).GetProperty("result").GetProperty("protocolVersion").GetString().ShouldBe("2025-06-18");

        await _mcp.HandleAsync(CallTool("subscribe", new { topic = "work" }), null, init.McpSessionId, null, default);
    ...
    {
        var init = await _mcp.HandleAsync(Rpc("initialize", new { protocolVersion = "2026-07-28" }), null, null, null, default);
    ...
    {
        var init = await _mcp.HandleAsync(Rpc("initialize", new { }), null, null, null, default);
        var id = init.McpSessionId;

        var list = await _mcp.HandleAsync(Rpc("tools/list"), null, id, null, default);
        ...
        await _mcp.HandleAsync(CallTool("subscribe", new { topic = "work" }), null, id, null, default);
        await _mcp.HandleAsync(CallTool("set_session_name", new { name = "api", working_dir = "/src/api" }), null, id, null, default);
        await _mcp.HandleAsync(CallTool("send_message", new { topic = "work", content = "renamed" }), null, id, null, default);

        _hub.GetTopics().Single().Subscribers.ShouldBe(["api"]);
        _hub.GetRecentMessages().Single().Sender.ShouldBe("api");

        var after = await _mcp.HandleAsync(Rpc("tools/list"), null, id, null, default);
    ...
    {
        var r = await _mcp.HandleAsync(Rpc("tools/list"), null, "gone", null, default);
    ...
    {
        var r = await _mcp.HandleAsync(Rpc("tools/list"), null, null, null, default);
    ...
    {
        var r = await _mcp.HandleAsync(JsonSerializer.Serialize(new { jsonrpc = "2.0", method = "notifications/initialized" }), "backend", null, null, default);
    ...
    {
        await _mcp.HandleAsync(CallTool("subscribe", new { topic = "work" }), "frontend", null, null, default);
        await _mcp.HandleAsync(CallTool("send_message", new { topic = "work", content = "hi" }), "backend", null, null, default);

        var r = await _mcp.HandleAsync(CallTool("list_topics", new { }), "frontend", null, null, default);
